...
document.getElementById('email-form').addEventListener('submit', async function (e) {
  ...
  try {
    const find_email = await fetch(`https://ndb.fut.ru/api/v2/tables/m2zenv52yinuxda/records/count?where=(tg-id,eq,${window.tgUserId})`, {
      method: 'GET',
      headers: {
    ...
    const res = await fetch(`https://ndb.fut.ru/api/v2/tables/m2zenv52yinuxda/records/count?where=(tg-id,eq,${window.tgUserId})~and(Результат КОТ,neq,-1)`, {
      method: 'GET',
      headers: {
        'accept': 'application/json',
        'xc-token': 'crDte8gB-CSZzNujzSsy9obQRqZYkY3SNp8wre88'
